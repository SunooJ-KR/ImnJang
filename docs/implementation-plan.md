...
| `data/collect/31` | 정비사업 구역 496건 | 재건축 추진 단지 150개 |
...
- `floor_band`: 단지 최고층 기준 1/3 이하 LOW, 2/3 이하 MID, 초과 HIGH. 최고층은 `23.3.horizon_profile`의 HIGH `repr_floor`를 proxy로 쓴다. profile이 없으면 `UNKNOWN`
...
**feature**: 30 스파이크의 M3 + 정비사업 신호와 동일한 집합을 쓴다. 스파이크에서 검증된 조합을 그대로 옮기는 것이므로 새로 고르지 않는다.
