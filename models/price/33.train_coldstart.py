...
def load_base():
    ...
    sale["area_type"], sale["floor_band"] = np.round(sale.excluUseAr/3)*3, assign_floor_band(sale, max_levels)
    ...
    return sale,complex_df,profile,max_levels,TEST_END
...
def score(name,train,test,rent=None,version=None,interaction=False,service_spec=True):
    levels=sorted(train.sgg_code.astype(str).unique()) if interaction else []
    feature_builder = service_model_features if service_spec else model_features
    tr,a=prepare(train,rent,version,TRAIN_END,interaction,levels); te,b=prepare(test,rent,version,TRAIN_END,interaction,levels); features=feature_builder(rent is not None,levels)
    ...
    return {"name":name,"version":version,"interaction":interaction,"family":family,"features":features,"levels":levels,"feature_spec":"SERVICE_M2_REDEVELOP" if service_spec else "I_M2_PHYSICAL_REDEVELOP","train":tr,"test":te,"oot":mape(te.price_per_m2,pt),"hold":mape(va.price_per_m2,ph),"bins":bin_mapes(va,ph),"n":len(va),"complexes":va.apt_seq.nunique(),"audit":a|{f"test_{k}":v for k,v in b.items()}}
...
def intervals(chosen):
    ...
    feature_builder = service_model_features if chosen["feature_spec"] == "SERVICE_M2_REDEVELOP" else model_features
    inner_features = feature_builder(chosen["version"] is not None, inner_levels)
...
def targets(complex_df,profile,max_levels,latest,rent):
    ...
    s=pd.read_csv(TRADES_PATH,sep="\t",low_memory=False).rename(columns={"aptSeq":"apt_seq"});s.apt_seq=s.apt_seq.astype(str);s["deal_period"]=pd.PeriodIndex(s.deal_ym.astype(str),freq="M");s["excluUseAr"]=pd.to_numeric(s.excluUseAr,errors="coerce");s=s.loc[s.apt_seq.isin(cold)&s.is_cancelled.astype("string").str.lower().ne("true")&s.excluUseAr.gt(0)&s.excluUseAr.le(300)&s.deal_period.between(latest-59,latest)].copy();s["area_type"]=np.round(s.excluUseAr/3)*3;s["floor_band"]=assign_floor_band(s,max_levels);s["area_type_source"]="SALE"
    r=rent.rent.loc[rent.rent.apt_seq.isin(cold)&rent.rent.deal_period.between(latest-59,latest),["apt_seq","area_type","floor","deal_period"]].copy();r["floor_band"]=assign_floor_band(r,max_levels);r["area_type_source"]="RENT"
