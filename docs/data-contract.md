...
`MODEL`은 전체 단지의 일부(2,081단지, 결정 68 적용 후)에만 붙는다. 예측구간의 명목 수준은 80%이고 검증 기간 실측 coverage는 84.42%다. **다만 "80% 보장"이라 쓰지 말 것** — 첫 coverage 실패를 보고 방식을 바꿨으므로 test가 재사용됐다. "개발 기간에서 관측된 값"까지만 말할 수 있다.
